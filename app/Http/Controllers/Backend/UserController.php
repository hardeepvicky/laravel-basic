...
class UserController extends BackendController
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            DB::commit();
            if (!$save_data['dont_send_email']) {
                $otp = rand(1000,9999);
                $model->otp = $otp;
                $model->otp_sent_datetime = now();
                $model->save();
                Mail::to($save_data['email'])->later(now()->addSeconds(2),new NewUserRegistered($otp,$model->name,Crypt::encryptString($model->email)));
            }
            $this->saveSqlLog();
